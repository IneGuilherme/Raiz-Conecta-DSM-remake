...
import { Leaf, Truck, TrendingUp, ArrowRight } from "lucide-react";

export default function HomePage() {
  const [roleLogado, setRoleLogado] = useState<string | null>(null);

  // Verifica quem está logado assim que a página carrega
  useEffect(() => {
    const carregarRole = () => {
  ...
  // Roteamento inteligente do Painel
  const getLinkDoPainel = () => {
    if (roleLogado === "admin") return "/admin";
    if (roleLogado === "mercado") return "/checkout";
    return "/produtor";
  };
          ...
          <div className="flex flex-col md:flex-row items-center gap-12">
            <div className="md:w-1/2 text-center md:text-left">
              ...
              <h3 className="text-lg md:text-xl text-gray-600 leading-relaxed font-light mb-8">
                ...
                alimentos. Conectando produtores e mercados para facilitar o
                acesso a alimentos frescos em um só lugar.
              </h3>

              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                {/* 👇 AQUI ESTÁ A INTELIGÊNCIA DOS BOTÕES 👇 */}
                {roleLogado ? (
                  <Link
                    href={getLinkDoPainel()}
                    className="px-8 py-3 bg-green-600 text-white rounded-lg font-medium shadow-lg hover:bg-green-700 transition-all text-center flex items-center justify-center gap-2"
                  >
            ...
            {[
              {
                ...
                text: "Reduza desperdícios e consuma de forma consciente.",
              },
              {
                icon: Truck,
                title: "Logística",
                text: "Entregas integradas entre produtores e mercados.",
              },
              {
                icon: TrendingUp,
                title: "Transparência",
                text: "Sistema de avaliação e controle de qualidade.",
              },
            ].map((item, idx) => (
              <div
                key={idx}
                className="bg-white text-center p-8 rounded-2xl shadow-md hover:shadow-lg transition-all"
              >
